...
router.patch("/rating/:mId", async (req, res) => {
  const mentorId = req.params.mId;
  try {
    const mentor = await User.findByIdAndUpdate(mentorId, { mentorRating: req.body.mentorRating });
...
router.get("/:interest", async (req, res) => {
  ...
  try {
    const users = await User.find({ isMentor: true, expertise: interest })
