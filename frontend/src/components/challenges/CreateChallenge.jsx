...
export default function CreateChallenge() {
  ...
  const handleChallenge = async (e) => {
    ...
    const challengeDate = new Date(dateTime).toString()
    ...
    const challengeBody = {
      ...
      endTime: challengeDate.substring(15, 55),
      endDate: challengeDate.substring(0, 15),
    }
    // console.log(room)
    try {
      setIsLoading(true)

      var challenge = await axios.post(
        `/challenge/create/${currentUser._id}`,
        challengeBody
      )
    } catch (error) {
      setIsLoading(false)
    ...
    window.location.replace(`/getchallenge/${challenge.data._id}`)
  }
  ...
  return (
    <div>
      <Helmet>
        <meta charSet="utf-8" />
        <title>Create a new challenge to connect with mentors | Eduwarts</title>
        <meta
          name="description"
          content="Learn and grow with help from your own skilled seniors"
        />
      </Helmet>
      <h2>Create a new challenge and publish this challenge to the community.</h2>
      <Box
        sx={{
          width: "100%",
            ...
            >
              <LocalizationProvider dateAdapter={AdapterDateFns}>
                <DateTimePicker
                  ...
                  label="Challeng end date and time"
                  value={dateTime}
                  onChange={(newValue) => {
