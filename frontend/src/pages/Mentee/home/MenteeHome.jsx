...
export default function MenteeHome() {
  ...
  useEffect(() => {
    const allMentors = async () => {
      ...
      setpreloader(true)
      const res = await axios.get(`/auth/${user.interests[0]}`,{headers})
      setMentorList(res.data);
      if(user.interests.length>1){
        let res2 =  await axios.get(`/auth${user.interests[1]}`,{headers})
        setMentorList(...mentorList,res2.data)

      }
      if(user.interests.length>2){
        const res3 = await axios.get(`/auth/${user.interests[2]}`)
        setMentorList(...mentorList, res3.data)
